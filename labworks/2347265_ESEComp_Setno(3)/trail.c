...
void displayPatients(struct PriorityQueue *pq) {
    ...
    for ( i = 0; i < pq->size; i++) {
        printf("PatientID: %d, Age: %d, Severity Level: %d\n",
               pq->patients[i].PatientID, pq->patients[i].Age, pq->patients[i].SeverityLevel);
    }
}
...
void displayHighestPriority(struct PriorityQueue *pq) {
    if (pq->size > 0) {
        ...
        printf("PatientID: %d, Age: %d, Severity Level: %d\n",
               pq->patients[0].PatientID, pq->patients[0].Age, pq->patients[0].SeverityLevel);
    } else {
